        ...
        <div className="w-full md:w-80 lg:w-96 border-l border-slate-200 bg-white flex flex-col z-20 shrink-0 shadow-[-10px_0_15px_-10px_rgba(0,0,0,0.05)]">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {(activeChat.adminId === currentUser?.id || (activeChat.deputyIds || []).includes(currentUser?.id)) && (
              <div className="p-4 border-b border-slate-100">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 px-2">Đổi hình nền</h4>
                <div className="grid grid-cols-4 gap-2 px-2">
                  {[
                    ...
                    <button
                      key={idx}
                      onClick={() => handleChangeGroupBackground(bg)}
                      className="w-full aspect-square rounded-lg border-2 border-transparent focus:border-blue-500 overflow-hidden"
                      style={bg ? { background: \`url(\${bg}) center/cover\` } : { backgroundColor: '#f8fafc', border: '1px solid #e2e8f0' }}
                    />
                  ))}
                </div>
              </div>
            ...
            <div className="p-4 space-y-2 pb-20">
              <button
                ...
                className="w-full py-3 text-red-600 font-bold hover:bg-red-50 rounded-xl transition-colors text-sm text-center px-4"
              >
                Rời nhóm
              </button>
