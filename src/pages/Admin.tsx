...
import { Plus, Trash2, Download, Building2, Users, ShieldAlert, Search, Database, Hexagon, Fingerprint, Lock, ShieldCheck, Terminal, Radar } from 'lucide-react';
import { QRCodeCanvas } from 'qrcode.react';
import { motion, AnimatePresence } from 'motion/react';

export default function Admin() {
  const { currentUser, rooms, addRoom, deleteRoom, classes, addClass, deleteClass, updateClass, users, updateUserRole, deleteUser } = useAppContext();
  ...
  if (currentUser?.role !== 'admin') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
         ...
         <p className="text-slate-500">Bạn không có quyền truy cập vào phân hệ quản trị cấp 2.</p>
      </div>
    );
  }
  ...
  const handleAddClass = (e: React.FormEvent) => {
    e.preventDefault();
    addClass({ ...newClass, schedule: [], announcements: [] });
  ...
  return (
    <div className="space-y-8 pb-20 relative">
        ...
        <div className="flex items-center gap-5 relative z-10">
           ...
           <div>
             ...
             <p className="text-slate-400 text-sm md:text-base font-medium mt-1 flex items-center gap-2">
               ...
               Phân hệ quản trị không gian mẫu / Giao thức bảo mật mã hóa cấp 2
             </p>
           </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 relative z-10 bg-white/60 p-2 rounded-2xl border border-white backdrop-blur-md w-fit shadow-lg">
        ...
        >
          <Building2 className={`w-4 h-4 ${activeTab === 'rooms' ? 'text-emerald-500' : ''}`} /> Quản Trị Module
        </button>
        <button
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          >
            <h2 className="text-xl font-display font-black text-slate-800 mb-6 flex items-center gap-3">
               ...
               Khởi tạo {activeTab === 'rooms' ? 'Module' : 'Nút Không Gian'}
            </h2>
            ...
            {activeTab === 'rooms' ? (
              ...
              <form onSubmit={handleAddClass} className="space-y-5">
                ...
                <div className="bg-amber-50 p-6 rounded-2xl border border-amber-100 text-center">
                   ...
                   <p className="text-sm text-slate-600">Thay đổi vai trò trực tiếp trên danh sách tài khoản.</p>
                </div>
            ) : (
          ...
          {activeTab === 'roles' ? (
            ...
            >
               ...
               <div className="p-8 border-b border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-6 relative z-10 bg-slate-900/50">
                 <h2 className="text-2xl font-display font-black text-white flex items-center gap-3">
                   <Fingerprint className="w-6 h-6 text-emerald-400" /> Bản Ghi Tài Khoản ({users.length})
                 </h2>
                ...
                <table className="w-full text-left text-sm border-separate border-spacing-y-2">
                  <thead>
                    <tr>
                      ...
                      <th className="p-3 font-black text-slate-500 uppercase tracking-widest text-xs w-56">Cấp Cấp Quyền</th>
                    </tr>
                  </thead>
                  <tbody>
                    <AnimatePresence>
                          ...
                          <td className="p-4 rounded-l-2xl border-y border-l border-white/5 group-hover:border-white/10 transition-colors">
                             <div className="flex items-center gap-4">
                                <div className="w-10 h-10 rounded-xl bg-slate-700 flex items-center justify-center text-slate-300 font-bold uppercase overflow-hidden shrink-0">
                                  {u.avatar ? <img src={u.avatar} className="w-full h-full object-cover" /> : (u.fullName || 'U').charAt(0)}
                                </div>
                                <div>
            ...
            >
              <h2 className="text-xl font-display font-black text-slate-800 mb-6 flex items-center gap-3">
                 ...
                 Archive {activeTab === 'rooms' ? 'Module Giảng Dạy' : 'Nút Lớp Học'} ({activeTab === 'rooms' ? rooms.length : classes.length})
              </h2>
              
              {(activeTab === 'rooms' && rooms.length === 0) || (activeTab === 'classes' && classes.length === 0) ? (
                ...
                <div className="space-y-4">
                  <AnimatePresence>
                  {(activeTab === 'rooms' ? rooms : classes).map((item, idx) => {
